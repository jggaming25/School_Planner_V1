-- SCHRITT 1: ALLES LÖSCHEN (einfach, ohne Referenzen)
DO $$ DECLARE r RECORD; BEGIN
  FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = 'public') LOOP
    EXECUTE 'DROP TABLE IF EXISTS public.' || quote_ident(r.tablename) || ' CASCADE';
  END LOOP;
  FOR r IN (SELECT policyname FROM pg_policies WHERE schemaname = 'public') LOOP
    EXECUTE 'DROP POLICY IF EXISTS ' || quote_ident(r.policyname) || ' ON public.' || (SELECT tablename FROM pg_policies WHERE policyname = r.policyname AND schemaname = 'public' LIMIT 1);
  END LOOP;
END $$;

-- SCHRITT 2: ALLE TABELLEN NEU ERSTELLEN
...
CREATE POLICY "Admins manage classes" ON classes FOR ALL USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('super_admin','admin','school_admin'))
);
