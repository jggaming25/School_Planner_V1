...
async function renderAbsenceApprovals() {
  if (!currentUser || !profile) return;
  if (!['teacher', 'school_admin', 'admin', 'supporter', 'head_admin', 'super_admin', 'ceo'].includes(profile.role)) return;
...
async function loadAbsenceApprovalsTab(status, tabBtn) {
  ...
  const isApprover = ['teacher', 'school_admin', 'admin', 'supporter', 'head_admin', 'super_admin', 'ceo'].includes(profile.role);
