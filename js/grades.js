...
function renderGradeSubjects() {
  ...
  el.innerHTML = entries.map(([sid, gs]) => {
    ...
    return `<div class="flex-between" style="padding:12px 0;border-bottom:1px solid var(--border-light)">
      <div class="flex gap-12" style="align-items:center"><div class="color-dot" style="background:${sub?.color || 'var(--accent)'}"></div><div><strong style="font-size:0.875rem">${escapeHtml(sub?.name || '?')}</strong><br><span class="text-muted" style="font-size:0.75rem">${gs.length} Note${gs.length !== 1 ? 'n' : ''}</span></div></div>
...
function renderGradeTable() {
  ...
  el.innerHTML = `<table><thead><tr><th>Fach</th><th>Note</th><th>Gewichtung</th><th>Typ</th><th>Datum</th>${profile.role !== 'student' ? '<th></th>' : ''}</tr></thead><tbody>${gradesData.map(g => {
